...
$repoDir = "$env:LOCALAPPDATA\MSE2_Shared_Cloud"
...
if (-not $setFile) {
    [System.Windows.Forms.MessageBox]::Show('Could not find the shared set file.','Import Set')
    exit
}

$xamlStr = '<Window xmlns=''http://schemas.microsoft.com/winfx/2006/xaml/presentation'' Title=''Import Set'' Width=''700'' Height=''570'' ResizeMode=''CanResize'' Background=''#0A0A14'' Foreground=''White'' FontFamily=''Segoe UI'' WindowStartupLocation=''CenterScreen''><Grid Margin=''18''><Grid.RowDefinitions><RowDefinition Height=''Auto''/><RowDefinition Height=''Auto''/><RowDefinition Height=''Auto''/><RowDefinition Height=''*''/><RowDefinition Height=''Auto''/></Grid.RowDefinitions><TextBlock Grid.Row=''0'' Text=''Import Cards from Another Set'' FontSize=''18'' FontWeight=''Bold'' Foreground=''White'' Margin=''0,0,0,14''/><StackPanel Grid.Row=''1'' Orientation=''Horizontal'' Margin=''0,0,0,10''><TextBlock Text=''Source .mse-set file:'' VerticalAlignment=''Center'' Margin=''0,0,10,0'' FontSize=''12'' Foreground=''#9CA3AF''/><TextBox Name=''PathBox'' Width=''330'' IsReadOnly=''True'' Background=''#111827'' Foreground=''#D1D5DB'' Padding=''6,4'' VerticalAlignment=''Center'' FontSize=''11''/><Button Name=''BtnBrowse'' Content=''Browse...'' Margin=''8,0,0,0'' Background=''#1E3A5F'' Foreground=''White'' Padding=''14,6'' FontSize=''12''/></StackPanel><TextBlock Name=''StatusText'' Grid.Row=''2'' Foreground=''#6B7280'' FontSize=''11'' Margin=''0,0,0,8'' TextWrapping=''Wrap''/><ScrollViewer Grid.Row=''3'' VerticalScrollBarVisibility=''Auto''><StackPanel Name=''PreviewList''/></ScrollViewer><Button Name=''BtnImport'' Grid.Row=''4'' Content=''Import Cards into Shared Set'' Margin=''0,12,0,0'' Background=''#1D4ED8'' Foreground=''#555'' FontWeight=''Bold'' FontSize=''13'' Padding=''20,10'' HorizontalAlignment=''Right'' IsEnabled=''False''/></Grid></Window>'
$xaml   = [xml]$xamlStr
...
function Stamp-Now([string]$block) {
    ...
    if ($block -match '(?m)(^\s*time_created:\s*[^\r\n]+)') {
        return $block -replace '(?m)(^\s*time_created:\s*[^\r\n]+)', ('' + "`n`ttime_modified: $now")
    }
    return $block
}
function Run-Preview([string]$srcPath) {
    $previewList.Children.Clear()
    $btnImport.IsEnabled = $false; $btnImport.Foreground = '#555'
    $statusText.Text = 'Reading...'
    $window.Dispatcher.Invoke([System.Windows.Threading.DispatcherPriority]::Render,[action]{})
    try {
        $srcTxt   = Read-ZipSet $srcPath
        if (-not $srcTxt)   { $statusText.Text = 'Cannot read selected file.'; return }
        $localTxt = Read-ZipSet $setFile.FullName
        if (-not $localTxt) { $statusText.Text = 'Cannot read shared set.'; return }
        $nameMap = @{}
        ...
        $cards = @($srcTxt -split '(?m)^(?=card:)' | Where-Object { $_ -match '^card:' } | ForEach-Object {
            ...
            $n = Get-CardName $blk
            if ($n) {
                $s = if ($nameMap[$n.ToLower()]) { 'OVERRIDE' } else { 'ADD' }
                [pscustomobject]@{ Block=$blk; Name=$n; Status=$s }
            }
        })
        if ($cards.Count -eq 0) { $statusText.Text = 'No cards found.'; return }
        $script:importCards = $cards
        $adds = ($cards | Where-Object Status -eq 'ADD').Count
        $ovrs = ($cards | Where-Object Status -eq 'OVERRIDE').Count
        ...
        foreach ($c in $cards) {
            ...
            $btb.FontSize = 10; $btb.FontWeight = 'Bold'; $btb.Foreground = 'White'
            ...
            $ntb.Text = $c.Name; $ntb.FontSize = 12; $ntb.Foreground = 'White'; $ntb.VerticalAlignment = 'Center'
            $sp.Children.Add($badge) | Out-Null
            $sp.Children.Add($ntb)   | Out-Null
            $row.Child = $sp
...
$btnImport.add_Click({
    ...
    try {
        $localTxt = Read-ZipSet $setFile.FullName
        if (-not $localTxt) { throw 'Cannot read shared set.' }
        $ordered = [System.Collections.Specialized.OrderedDictionary]::new()
        $localTxt -split '(?m)^(?=card:)' | Where-Object { $_ -match '^card:' } | ForEach-Object {
            $blk = ($_ -split '(?m)^(?=keyword:|version_control:|apprentice_code:)')[0]
            $n = Get-CardName $blk
            if ($n -and -not $ordered.Contains($n.ToLower())) { $ordered[$n.ToLower()] = $blk }
        }
        $addN = 0; $ovrN = 0
        foreach ($c in $script:importCards) {
            $key = $c.Name.ToLower(); $stmp = Stamp-Now $c.Block
            if ($ordered.Contains($key)) { $ordered[$key] = $stmp; $ovrN++ }
            else                          { $ordered[$key] = $stmp; $addN++ }
        }
        $hIdx = $localTxt.IndexOf("`ncard:")
        $header = if ($hIdx -ge 0) { $localTxt.Substring(0,$hIdx+1) } else { '' }
        $lIdx = $localTxt.LastIndexOf("`ncard:")
        $trail = ''
        if ($lIdx -ge 0) {
        ...
        $out = $header
        foreach ($k in $ordered.Keys) { $out += $ordered[$k].TrimEnd() + "`r`n" }
        $out += $trail
        ...
        Copy-Item $tmp $setFile.FullName -Force
        Remove-Item $tmp -Force -ErrorAction SilentlyContinue
        $statusText.Text = "Done! +$addN added, ~$ovrN overridden. Now use Cloud Sync to upload."
        ...
        $statusText.Foreground = $conv.ConvertFromString('#F87171')
